...
export default function EditListingSizeScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        (data as {
          ...
          const title = getSectionTitle(row.gender, row.type, t);
          ...
          if (!bySectionTitle[title]) bySectionTitle[title] = [];
    ...
    void load();
  }, [t, values.categoryGender, values.categoryType, values.category?.id]);
  ...
  const handleConfirm = () => {
    ...
    setField('size', { id: selectedId, label: foundLabel });
    router.back();
  };
          ...
          {loading ? (
            ...
            <ScrollView contentContainerStyle={styles.list}>
              {sections.map((section, index) => (
                <View key={`${section.title ?? 'section'}-${index}`} style={index > 0 ? styles.sectionSpacing : undefined}>
                  {section.title && (
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { color: '#999999', fontSize: 13, paddingTop: 20, paddingBottom: 8 },
  row: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  ...
  rowStock: { ...theme.typography.body, fontSize: 16, color: '#AAAAAA' },
  radioOuter: {
    width: 22,
    height: 22,
    borderRadius: 11,
