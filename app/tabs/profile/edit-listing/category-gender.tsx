...
export default function EditListingCategoryGenderScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setCategories(
          (data as { id: number; name: string; slug: string; gender: string | null }[]).map(
            (row) => ({
              id: row.id,
              name: row.name,
              slug: row.slug,
  ...
  const openDetail = (category: RootCategory) => {
    router.push({
      pathname: '/tabs/profile/edit-listing/category-detail',
      params: {
